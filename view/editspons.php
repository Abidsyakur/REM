<?php
    ...
    $sponsor = mysqli_query($conn,"SELECT program_sponsor.id_program_sponsor, program.id_program,program.nama_program, sponsor.nama_sponsor
    FROM program_sponsor
    JOIN program ON program_sponsor.id_program = program.id_program
    JOIN sponsor ON program_sponsor.id_sponsor = sponsor.id_sponsor
    WHERE program_sponsor.id_program_sponsor = $id_program_sponsor");
    $result = mysqli_fetch_array($sponsor);
...
<html lang="en">
  <head>
    ...
    <title><?=$result['nama_sponsor']?></title>
...
button[type="submit"]:hover {
  ...
  <a href="#">RAMADHAN EVENT</a>
    ...
    <div class="container" style="margin-top:2rem;margin-bottom:4rem;">
        ...
        <form action="../func/editspons.php" method="POST">
  <input type="text" value="<?=$result['id_program_sponsor']?>" hidden name="id_program_sponsor" id="id_program_sponsor">
  <select name="id_program" class="form-select" aria-label="Default select example">
  <option value="<?=$result['id_program']?>"><?=$result['nama_program']?></option>
  <?php foreach($program as $prog) : ?>
  <option value="<?= $prog['id_program'] ?>"><?=$prog['nama_program'] ?></option>
  <?php endforeach; ?>
</select>
  <button type="submit" id="submit" name="submit">SPONSOR</button>
</form>
    </div>
