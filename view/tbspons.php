...
$sponsor = mysqli_query($conn,"SELECT program_sponsor.id_program_sponsor, program.nama_program, sponsor.nama_sponsor
...
WHERE program_sponsor.id_sponsor = $id_sponsor");
$hasil = mysqli_fetch_array($sponsor);
?>
...
.btn-link:hover {
  ...
  <tbody>
    <?php $no=1; foreach($sponsor as $sp):?>
    <tr>
        <th scope="row"><?=$no++?></th>
        <td><?= $sp['nama_program']?></td>
        <td><?= $sp['nama_sponsor']?></td>
        <td>
        <a href="editspons.php?id=<?=$sp['id_program_sponsor'];?>" class="btn-link">Edit</a>
        <a href="../func/delspons.php?id=<?=$sp['id_program_sponsor'];?>" class="btn-link">Delete</a>
        </td>
    </tr>
    <?php endforeach; ?>
  </tbody>
